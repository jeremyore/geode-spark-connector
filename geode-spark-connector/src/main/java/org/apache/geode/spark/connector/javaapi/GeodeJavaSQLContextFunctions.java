...
public class GeodeJavaSQLContextFunctions {
  ...
  public <T> Dataset geodeOQL(String query) {
    Dataset ds = scf.geodeOQL(query, scf.defaultConnectionConf());
    return ds;
  }

  public <T> Dataset geodeOQL(String query, GeodeConnectionConf connConf) {
    Dataset ds = scf.geodeOQL(query, connConf);
    return ds;
  }
